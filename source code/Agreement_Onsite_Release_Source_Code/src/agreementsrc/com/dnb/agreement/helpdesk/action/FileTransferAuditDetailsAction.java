...
import com.dnb.agreement.helpdesk.bean.FileTransferAuditBean;
import com.dnb.common.exception.CommonSystemException;
import com.dnb.common.session.SessionDataHandler;

public class FileTransferAuditDetailsAction extends Action 
{
	static Logger logger = Logger.getLogger(FileTransferAuditDetailsAction.class);
	...
	throws AgreementSystemException,AgreementBusinessException,CommonSystemException {
		...
		FileTransferAuditDTO edto = null;
		FileTransferAuditBean ea = null;
		...
		try
		{
			...
			ea = (FileTransferAuditBean) form; 
			edto = new FileTransferAuditDTO();
			...
			rep_ret= entDAO.showFileTransferAuditDetails(edto,userId,dateFormat);		
			...
			if(rep_ret ==  "ERR-RPT-001" || rep_ret.equals("ERR-RPT-001"))	{
				...
				request.setAttribute("agreementId",edto.getAgreementId());
				request.setAttribute("serviceType",edto.getServiceType());
				
				request.setAttribute("dateFrom",edto.getDateFrom());
				request.setAttribute("dateTo",edto.getDateTo());
				request.setAttribute("groupBy",edto.getGroupBy());
				
				request.setAttribute("hoursFrom",edto.getHoursFrom());
				request.setAttribute("hoursTo",edto.getHoursTo());			
