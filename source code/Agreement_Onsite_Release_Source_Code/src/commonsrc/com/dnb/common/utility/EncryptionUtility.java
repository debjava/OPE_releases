...
public class EncryptionUtility
{
    ...
    public EncryptionUtility()
    {
        ...
        {
        	ex.printStackTrace();
            logger.debug("EncryptionUtility initializion failed : "+ex);
    ...
    {
    	System.out.println("EncryptionUtility : encrypt - plainText ---->>>"+plainText);
		byte[] encoded = null;
        ...
        try
        {
        	System.out.println("Entering into encryption........before logger");
//			logger.debug("You are in encrypt method");
			System.out.println("chars2 bytes........");
			byte[] bytes = chars2Bytes(plainText.toCharArray());
            System.out.println("EncryptionUtility : encrypt - bytes ---->>>"+bytes);
            byte[] encrypted = doCipher(bytes, true);
            System.out.println("EncryptionUtility : encrypt - encrypted ---->>>"+encrypted);
            ...
            System.out.println("EncryptionUtility : encrypt - value of encoded ------>>>"+encoded);
            System.out.println("Encryption ended...........");
        }
        catch (GeneralSecurityException ex)
        {
//            logger.debug("Unable to encrypt string '" + plainText + "'", ex);
        	System.out.println("Unable to encrypt string ");
            ex.printStackTrace();
        }
        catch( Exception e )
        {
    ...
    public String decrypt(String input)
    {
        ...
        {
//            logger.debug("Unable to decrypt string '" + input + "' : "+ ex);
            ex.printStackTrace();
        }
        catch( Exception e )
        {
    ...
    {
    	System.out.println("-------------- EncryptionUtility : doCipher ---------");
        SecretKey desKey = keyFactory.generateSecret(desedeKeySpec);
        System.out.println("EncryptionUtility : doCipher - SecretKey -------->>>"+desKey);
        if (!encrypt)
        {
            desCipher.init(Cipher.DECRYPT_MODE, desKey);
            System.out.println("EncryptionUtility : doCipher - inside decrypt mode");
        }
        else
        {
            desCipher.init(Cipher.ENCRYPT_MODE, desKey);
            System.out.println("EncryptionUtility : doCipher - inside encrypt mode");
        }
