 ...
 *																							*
 * File Name                   : DescriptorUtility.java                                      *
 * Author                      : Raja Mohan                                                  *
 * Creation Date               : 28-Apr-2006                                                 *
 * Description                 : Utility which gets the OracleConnection object from         *
 *                               underlying connection objects and has utility methods to    *
 *                               get the ARRAY and STRUCT descriptor                         *
...
public class DescriptorUtility {
	 ...
	 * @throws SQLException
	 */
	public static Array getArrayOfStructs(String arrayTypeName,
			String structName, Connection conn, List arrayContents)
			throws SQLException {
		Array array = null;
		StructDescriptor descriptor = StructDescriptor.createDescriptor(
				structName, conn);
		...
		for (int i = 0; i < arrayContents.size(); i++) {
			structArr[i] = new STRUCT(descriptor, conn,
					(Object[]) arrayContents.get(i));
		} // endfor i
	 ...
	 *             In case of DB error
	 */
	public static StructDescriptor getStructDescriptor(String name,
	 ...
	 *             In case of DB error
	 */
	public static ArrayDescriptor getArrayDescriptor(String name,
			Connection conn) throws SQLException {
		OracleConnection oConn = getOracleConnection(conn);
	...
	public static Clob createClob(Connection conn, String string)
			throws SQLException {
		OracleConnection oConn = getOracleConnection(conn);
		...
		if (loaded) {
			...
			try {
				oConn = (OracleConnection) extractionMethod.invoke(conn,
						passConnection ? new Object[] { conn }
								: new Object[] {});
			...
			// this can throw the infamous pm-runtime-exception
	...
	private static OracleConnection getNativeConnection(Connection conn)
		...
		try {
			// websphere 6
			tryOut = Class
					.forName("com.ibm.ws.rsadapter.jdbc.WSJdbcConnection");
		...
		try {
			...
			tryOut = Class
					.forName("org.jboss.resource.adapter.jdbc.WrappedConnection");
			if (tryOut.isInstance(conn)) {
		...
		try {
			...
			tryOut = Class
					.forName("org.apache.commons.dbcp.DelegatingConnection");
			if (tryOut.isInstance(conn)) {
		...
		throw new CommonRuntimeException("", "Unknown connection type");
	}

	private static OracleConnection getDbcpNative(Connection conn,
			Class wrapperClass) {
		OracleConnection result = null;
		try {
			Method m = wrapperClass.getMethod("getInnermostDelegate",
					new Class[] {});
	...
	private static OracleConnection getJbossNative(Connection conn,
			Class wrapperClass) {
		OracleConnection result = null;
		try {
			System.out.println("!!!!!!!!!!!!jbossNative method");
			Method m = wrapperClass.getMethod("getUnderlyingConnection",
					new Class[] {});
			result = (OracleConnection) m.invoke(conn, new Object[] {});
			init(m, false, true);
		} catch (SecurityException e) {
	...
	private static OracleConnection getWebSphereNative(Connection conn) {
		...
		try {
			...
			Method m = util.getMethod("getNativeConnection", new Class[] { conn
					.getClass() });
			if (m != null) {
				result = (OracleConnection) m.invoke(null,
						new Object[] { conn });
			}
			init(m, true, true);
		} catch (ClassNotFoundException e) {
	...
	private static void init(Method eMethod, boolean passCon, boolean cLoaded) {
