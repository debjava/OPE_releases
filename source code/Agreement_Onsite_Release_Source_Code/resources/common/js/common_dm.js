...
function addTab(title,url)
{
	...
	var tab='<a href="'+url+'" id="'+tabID+'" class="tab active"><span class="shadow">'+title+'</span><span class="text">'+title+'</span><span class="close"></span></a>';
...
{	
	var wcode='<div class="window-bg"></div><div class="window" style="{width:'+width+';}"><div class="title"><span class="shadow">'+title+'</span><span class="text">'+title+'</span><a href="#" class="close">X</a></div><div class="window-content" style="{height:'+height+';}"><div class="window-loader"></div></div></div>';
	$('body').prepend (wcode);
	if ($.browser.msie && Number($.browser.version)<=6) {
		hideSelects();
		$('.window-bg').height( $(document).height() );
