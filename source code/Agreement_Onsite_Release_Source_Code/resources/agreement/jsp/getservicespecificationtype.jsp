...
function clearBureauName(obj){
 ...
 function addNewService()  
 {
	 ...
	 var rowNo= opener.document.getElementById('rowNo').value;
...
function ajaxReturnToFunctionServiceId(xmlEnterpriseDates)
{
	...
	{	
		...
		setFlags();
		addNewService();
		document.getservicespecifictypenewform.submit();
		return true;
		}
	}
}
...
<html:form action="/resources/agreement/jsp/getservicespecifictypenew">
...
<ul class="toolbar2">    
	...
	<c:set var="NEW" value="${fn:substring(rights, 0, 1)}" />   
	 <li class="save">
	   <c:choose>	 
		<c:when test="${NEW=='1'}">
		 <a onClick="javascript: validateService('NEW')" 
		  onMouseOver="hover('a1','ahand');" tabindex="1" onkeyPress="saveAfterEnterKey();"
...
<input type="hidden" name="rowNo" value="${param.rowNo}" id="rowNo">
  ...
  <table width="96%">
		...
		<tr>
			...
			<td><span class="asterisk">*</span><select name="bureauId" id="${service_bureau_id}" onchange="getAjaxValueForServiceBureauName(this.value,'GET_BUREAU_NAME','${getBureauNamePath}');clearBureauName(this);" >
				   ...
				   <c:forEach items="${bureauIds}" var="bureauIds">
						   ...
						   <c:out value="${bureauIds.value}"/></option>
			       </c:forEach>		   
			 </select> 
			 </td>
		</tr>
	   ...
	   <c:forEach items="${bureauNames}" var="bureauName1">       
		...
		<tr>
			...
			<c:set var="service_bureau_name"><fmt:message key="service_bureau_name"/></c:set>
			 <td><span class="asterisk">&nbsp;&nbsp;</span><input type="text" size="30" name="bureauName" id="${service_bureau_name}" value="${bureau_Name}" readonly="true"></td>
		</tr>
			...
			<table>
				...
				<c:choose>
					<c:when test="${serviceList.suo eq 'Y'}">
						<span class="asterisk">&nbsp;</span><input type="radio" name="suo" checked class="radio" value="Y" id="${suo_var}" >
					</c:when>
					<c:otherwise>
						<span class="asterisk">&nbsp;</span><input type="radio" name="suo" class="radio" value="Y" id="${suo_var}" >
					</c:otherwise>
		       </c:choose>
		       ...
		       <c:choose>
			      <c:when test="${serviceList.suo eq 'N'}">
		           <input type="radio" name="suo" checked class="radio" value="N" id="${suo_var}" >
				  </c:when>
			      <c:otherwise>
				   <input type="radio" name="suo" class="radio" value="N" id="${suo_var}" >
				  </c:otherwise>
		       </c:choose>
   ...
   <!-- this row add and delete rows for customer information-->
		...
		<fieldset class="fieldset">
			...
			<div id="agreement_table_scrollbar">
				  ...
				  <c:forEach items='${customerDetails}' var='customerList' varStatus="status">  
					  <tr>
						<td>
							<input type="text" readonly="true" name="formItems[<c:out value="${status.count-1}"/>].customerId" id="formItems[<c:out value='${status.count-1}'/>].<fmt:message key="fcc_customer_id"/>*" maxlength="14" size="33" onblur="javascript: this.value=Trim(this.value);this.value=this.value.toUpperCase();
							getAjaxValueForCustomer(this);" value="${customerList.customerId}"
							onkeypress="return alphanumericWithoutSpace(this);"/>
						</td>
						<td>
							<input type="text" readonly="true" name="formItems[<c:out value="${status.count-1}"/>].customerName" id="formItems[<c:out value="${status.count-1}"/>].<fmt:message key="customer_name"/>" maxlength="40" 
							value="${customerList.customerName}"/>
						</td>
						
						<td>	
							<input type="button" title='<fmt:message key="delete"/>' name="delete[<c:out value="${status.count-1}"/>]" class="button"
		...
		<fieldset class="fieldset">
			...
			<div id="service_table_scrollbar"> 
	   		<table width="62%" border="0" id="accountsTab">
				...
				<tr>
				<td>
					<input type="text" readonly="true" name="formItems1[<c:out value="${status.count-1}"/>].accountNum" maxlength="14" size="33"
				...
				<td>
					<input type="text" readonly="true" name="formItems1[<c:out value="${status.count-1}"/>].accountName" maxlength="32" 
