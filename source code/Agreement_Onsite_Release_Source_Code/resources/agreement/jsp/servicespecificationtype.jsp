...
function ajaxReturnToFunctionServiceId(xmlEnterpriseDates)
{
	...
	{	
		...
		setFlags();
		addNewService();
		document.servicespecifictypenewform.submit();
		return true;
		}
	}
}
...
function clearAfterEnterKey()
{
	...
	if(key==13)
	{
		document.servicespecifictypenewform.reset();
		afterClear();
	}
}
...
<html:form action="/resources/agreement/jsp/servicespecifictypenew" > 
<%@ include file="common/JavascriptErrorMsg.jsp" %>

<input type="hidden" name="editorHold" Id="editorHold" value="NEW"/>
...
<ul class="toolbar2">    
	<!-- Subnavigation -->		
	  ...
	  <li class="clear">
		  <a onClick="javascript:document.servicespecifictypenewform.reset();afterClear();"
		...
		<tr>
			...
			<td><span class="asterisk">*</span><select name="bureauId" id="${service_bureau_id}" onchange="getAjaxValueForServiceBureauName(this.value,'GET_BUREAU_NAME','${getBureauNamePath}');clearBureauName(this);" >
				   ...
				   <c:forEach items="${bureauIds}" var="bureauIds">
						   ...
						   <c:set var="display_bureau_id"><c:out value="${bureauIds.value}"/></c:set>
				   </c:forEach>		   
				</select> 
			</td> 

			<input type="hidden" name="displayId" id="displayId" />
		</tr>
			...
			<c:url value="/resources/agreement/jsp/getCustomerInformation.do" var="getCustomerInformationPath"/>
			  ...
			  <table border="0" width="70%" id="tab">
				  <tr>
					...
					<td><input type="button" title='<fmt:message key="delete"/>' name="delete[0]" id="0" class="button" value=" - " onclick="removeRowFromTable(this,this);" /></td>	
				  </tr>
			  </table>
			  </div>
		</fieldset>
   ...
   <tr>
		...
		<fieldset class="fieldset">
			...
			<tr>
				...
				<td><input type="button" title='<fmt:message key="delete"/>' class="button" name="delete[0]" id="0" value=" - " onclick="removeRowFromTable1(this,this);" /></td>
			</tr>
			</table>
			</div> 
		</fieldset>
