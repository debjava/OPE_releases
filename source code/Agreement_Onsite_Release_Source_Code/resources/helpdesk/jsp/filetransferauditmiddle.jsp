...
function call()
{
	...
	report = rpt +"filetransferauditdetails"+randomnumber+".rptdocument";
		
	document.FileTransferAuditDetailsSearch.method="POST";

	document.FileTransferAuditDetailsSearch.action="<c:out value="${pageContext.request.contextPath}"/>/frameset?__report=resources/helpdesk/reportdesigns/FileTransferAuditDetails.rptdesign&__overwrite=true&__document="+report+"&CurrentUser="+currentUser+"&userSystemDate="+userSystemDate+"&dataMatriceImageUrl="+dataMatriceImageUrl+"&clientLogoImageUrl="+clientLogoImageUrl+"&Branch="+currentBranch+"&fetchStatus="+fetchStatus;
...
<form name="FileTransferAuditDetailsSearch" action="">	
	...
	<html:hidden property="agreementId" value="${requestScope.agreementId}"/>

	<html:hidden property="serviceType" value="${requestScope.serviceType}"/>

	<html:hidden property="dateFrom"  value="${requestScope.dateFrom}"/>
	<html:hidden property="dateTo" value="${requestScope.dateTo}"/>

	<html:hidden property="hoursFrom"  value="${requestScope.hoursFrom}"/>
	<html:hidden property="hoursTo" value="${requestScope.hoursTo}"/>
